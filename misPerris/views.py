...
def detalle_mascota(request, id):
    ...
    galeria_img = Galeria.objects.filter(mascota=mas)
    contexto["galeria"] = galeria_img
    cantidad = Galeria.objects.filter(mascota=mas).count()
...
def info_mascota(request, id):
    ...
    cantidad = Galeria.objects.filter(mascota=mas).count()
    contexto["cantidad"] = cantidad
    return render(request,"info_mascota.html", contexto)
...
def formulario(request):
    ...
    contexto={"tipos":tipos,"mascotas":mascotas,"mensaje":mensaje,"cant":cant}
    return render(request, "formulario.html",contexto)
...
def insertar_imagen(request):
    ...
    if request.POST:
        ...
        imagen_mascota = request.FILES.get("txtImg")
        obj_mascota = Mascota.objects.get(nombre=nombre_mascota)
        
        if imagen_mascota is not None:
            gale = Galeria()
            gale.imagen= imagen_mascota
            gale.mascota= obj_mascota
            gale.save()
            mensaje="Grabo Imagen"
        else:
            mensaje="no hay imagen para mascota "+nombre_mascota
...
def buscar_nombre(request):
    mensaje=""
    if request.POST:
        nom_mascota = request.POST.get("txtNombre")
        try:
            mas = Mascota.objects.get(nombre= nom_mascota)
            contexto = {"mascota":mas}
            galeria_img = Galeria.objects.filter(mascota=mas)
            contexto["galeria"] = galeria_img
            cantidad = Galeria.objects.filter(mascota=mas).count()
            ...
            mensaje="No existe la mascota "+nom_mascota
